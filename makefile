...
CFLAGS   += -DWINTITLE='"$(TITLE) $(VERSION)"'
LDLIBS   := `pkg-config --libs gtk+-3.0 libconfig`

ifeq ($(OS), Windows_NT)
CFLAGS   += -DMIAHIDPORDO_DATA_DIR='"."'
else ifeq ($(shell uname -s), Linux)
CFLAGS   += -DMIAHIDPORDO_DATA_DIR='"/usr/share/$(TARGET)"'
endif
...
ifeq ($(OS), Windows_NT)
	windres -I. -i windows/application/resource.rc -o resource.o
endif
...
deb:
	...
	@install -m 0644 window1.glade $(STAGE)$(PREFIX)/share/$(TARGET)
